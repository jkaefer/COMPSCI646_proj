...
TRACE = "/content/drive/MyDrive/mRAG_and_MSRS_source/agent_server_log/server_llm_trace.jsonl"
...
class SeverLLM:
    ...
    def generate(self, messages: list, sampling_params: SamplingParams = SamplingParams()):
        ...
        while True:
            try:
                ...
                error_msg = str(e)
                full_msg = f"Error in SeverLLM.generate: {error_type}: {error_msg}"
                jlog(TRACE, "server_llm_base_url", raw_preview=self.base_url)
                jlog(TRACE, "server_llm_try_error", raw_preview=full_msg)
                print(self.model)
    ...
    def post_process(self, response):
        ...
        for choice in response.choices:
            ...
            jlog(TRACE, "attempted json output", raw_preview=content)
            if self.assume_json:
